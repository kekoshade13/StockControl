<div class="nav-MenuVert">
                <nav class="navbar navbar-expand d-flex flex-column align-items-start" id="sidebar">
                <img src="assets/img/logos/logoprin.png" alt="" width="200" height="150">
                    <a href="index.php" class="navbar-brand text-light d-block mx-auto">
                        <div class="display-6" style="font-size: 30px;">StockControl
                        </div>
                    </a>
                    <ul class="navbar-nav d-flex flex-column mt-5 w-100" style="margin-top: -20%;">
                        <li class="nav-item w-100 mt-1">
                            <a href="ordenes.php" class="nav-link text-light pl-4">OT Reparaciones</a>
                        </li>
                        <li class="nav-item w-100 mt-3">
                            <a href="movimientos.php" class="nav-link text-light pl-4">Movimientos</a>
                        </li>
                        <li class="nav-item w-100 mt-3">
                            <a href="inventario.php" class="nav-link text-light pl-4">Inventario</a>
                        </li>
                        <?php if ($dataUser->class == "Admin") { ?>
                        <li class="nav-item w-100" style="margin-top: 50%;">
                            <a href="admin/index.php" class="nav-link text-light pl-4">Administrador</a>
                        </li>
                        <?php } ?>
                    </ul>
                </nav>  
        </div>